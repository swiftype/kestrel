...
class SwiftypeTimeSeriesCollectorSpec extends SpecificationWithJUnit {
  "SwiftypeTimeSeriesCollector" should {
    var collector: SwiftypeTimeSeriesCollector = null

    doBefore {
      Stats.clearAll()
      collector = new SwiftypeTimeSeriesCollector()
    }
    ...
    "pruneHourly()" in {
      Time.withCurrentTimeFrozen { time =>
        ...
        try {
          ...
          fail("Expected counter to be removed by pruneHourly()")
        } catch {
